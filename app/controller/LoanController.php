<?php
	...
	class LoanController extends Controller {
		// 여신관리
		function loan () {
			$this->cntcode = isset($_GET['back']) ? $_GET['back'] : $this->model->get_next_idx("kcntmp");
		...
		function sch_insert () {
			...
			for ($i = 1; $i <= $this->cs_data->cntpym; $i++) {
				...
				$this->model->query("INSERT INTO kamtmp SET cntnum = ?, amtodr = ?, amtdue = ?, rntamt = ?, prnamt = ?, intamt = ?, prnmal = ?, bildat = now()", [$_GET['cntnum'], $i, $amtdue, $rntamt, $rntamt - $intamt, $intamt, $acqcst]);
			}
			alert('생성되었습니다.');
			move(HOME."/loan");
		}
